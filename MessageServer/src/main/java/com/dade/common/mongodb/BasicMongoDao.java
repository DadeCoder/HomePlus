...
public abstract class BasicMongoDao< T extends MongoObject> {
    ...
    public void upsert(String id, T objectToUpsert) {
        DBObject dbObject = (DBObject) mongoOperations.getConverter().convertToMongoType(objectToUpsert);
        Update setUpdate = Update.fromDBObject(new BasicDBObject("$set", dbObject));
        Query query = Query.query(Criteria.where("id").is(id));

        LogUtil.info("upsert query=" + query.toString() +
                ", update= " + setUpdate.toString() +
                ", class= " + getClass().getSimpleName());

        mongoOperations.upsert(query, setUpdate, getReturnClass());
    }

    public void upsert(String key, Object value, T objectToUpsert) {
        DBObject dbObject = (DBObject) mongoOperations.getConverter().convertToMongoType(objectToUpsert);
        Update setUpdate = Update.fromDBObject(new BasicDBObject("$set", dbObject));
        Query query = Query.query(Criteria.where(key).is(value));

        LogUtil.info("query=" + query.toString() +
                ", update= " + setUpdate.toString() +
                ", class= " + getClass().getSimpleName());
